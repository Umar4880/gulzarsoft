...
class communication:
    ...
    @tool
    def web_search(self):
        #for now a mock websearch tool
        return "server not responding!!!"
    
    #------------------------------utitlity---------------------
    def _research_agent(self, state:researcher_state):
        system_prompt = (
        ...
        react_agent = create_agent(model=self.model, tools=[self.web_search])

        chain = template | react_agent | self.parser

        result = chain.invoke({"query":state['query']})

        return {"answer":result}
    
    def should_continue(self, state: researcher_state):
        if hasattr(state['answer'], 'tool_calls') or state['answer'] == 'tool_calls':
            return 'tools'
        return 'end'
    
    def _build_researcher_graph(self):
        graph = StateGraph(researcher_state)

        graph.add_node('agent', self._research_agent)
        graph.add_node('tools', ToolNode[self.web_search])
    ...
    def _researcher(self, state:researcher_state):
        research_graph = self._build_researcher_graph()

        result = research_graph.invoke(state['query'])
