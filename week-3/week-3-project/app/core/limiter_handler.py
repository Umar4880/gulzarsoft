...
class RateLimitCallbackHandler(AsyncCallbackHandler):
    ...
    async def on_chat_model_start(
        ...
        **kwargs: Any,
        ...
        """
        # Flatten all message content for token estimation
        prompt_text = " ".join(
            msg.content
        ...
        # Acquire RPM slot — this will delay/block if limit is hit
        await self._rpm_limiter.acquire(self.user_id)

        # Reserve estimated input tokens in TPM bucket
        await self._tpm_limiter.acquire(self.user_id, weight=self._reserved_tokens)

    # ------------------------------------------------------------------ #
    #  POST-CALL HOOK — SETTLEMENT                                         #
    # ------------------------------------------------------------------ #
